...
КонецПроцедуры // УстановитьОтображениеПланировщика()
...
Процедура ЗаполнитьПланировщикНаСервере()
	...
	|	ЕСТЬNULL(РеализацияТоваровИУслуг.СтатусОплаты, 0) КАК СтатусОплаты
	|ИЗ
	|	Документ.ЗаписьКлиента КАК ЗаписьКлиента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровИУслуг КАК РеализацияТоваровИУслуг
	|		ПО РеализацияТоваровИУслуг.ДокументОснование = ЗаписьКлиента.Ссылка
	|ГДЕ
	...
	Пока ВыборкаСотрудники.Следующий() Цикл 
		...
		Пока Выборка.Следующий() Цикл 
			...
			СоответсвиеЗначений = Новый Соответствие;
			СоответсвиеЗначений.Вставить("Сотрудники", Выборка.Сотрудник);
			...
			Если ЗначениеЗаполнено(Выборка.СтатусОплаты)
				И Выборка.СтатусОплаты = Перечисления.ВидыОплатыДокумента.ПолностьюОплачен Тогда 
			...
			НовыйЭлемент.ЗначенияИзмерений = Новый ФиксированноеСоответствие(СоответсвиеЗначений);
...
КонецПроцедуры // УстановитьПредставлениеПериода()
...
Процедура ПланировщикПередНачаломБыстрогоРедактирования(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.ЗаписьКлиента.ФормаОбъекта", ПараметрыОткрытияФормы,, ЭтаФорма);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Заказ записан"  Тогда 
